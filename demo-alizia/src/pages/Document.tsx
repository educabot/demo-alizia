...
export function Document() {
  ...
  const handleGenerateContent = async () => {
    ...
    try {
      // Generate strategy, problem_edge, eval_criteria
  ...
  const hasClassPlans = subjects.some((s: any) => {
    const sData = subjectsData[s.id] || {};
    return (sData.class_plan || []).length > 0;
  });
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
              {isGeneratingClasses ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center px-6 py-16">
                  ...
                  <h4 className="headline-1-bold text-[#10182B] mb-2">Planifiquemos nuestras clases</h4>
                  <p className="body-2-regular text-[#47566C] mb-6 max-w-[220px]">
                    Generá automáticamente el plan de clases para cada disciplina con IA
                  </p>
                  <Button
                    onClick={handleGenerateClasses}
                    disabled={isGenerating}
                    className="bg-primary text-white rounded-xl px-6 py-2.5 cursor-pointer hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Generar
                  </Button>
                </div>
