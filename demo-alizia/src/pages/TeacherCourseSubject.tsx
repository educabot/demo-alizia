...
export function TeacherCourseSubject() {
  ...
  const getClassesPerWeek = (): number => {
    if (!course?.schedule || !cs) return 1;
    const days = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday'] as const;
    let count = 0;
    for (const day of days) {
      const slots = (course.schedule as any)[day] || [];
      for (const slot of slots) {
        if (slot.subject === cs.subject_name) {
          count++;
        }
      }
    }
  ...
  const transformToDocumentSections = (): DocumentSection[] => {
    ...
    const classesPerWeek = getClassesPerWeek();
    const classesByWeek: Record<string, any[]> = {};

    coordStatus.class_plan.forEach((c: any) => {
      const weekNumber = Math.ceil(c.class_number / classesPerWeek);
      const weekName = `Semana ${weekNumber}`;

      if (!classesByWeek[weekName]) {
        classesByWeek[weekName] = [];
      }
      classesByWeek[weekName].push(c);
    });

    return Object.entries(classesByWeek).map(([name, classes], index) => ({
      id: index + 1,
      name,
